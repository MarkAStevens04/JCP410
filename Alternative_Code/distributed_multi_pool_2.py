def thread_run(trial_num, seed, queue, alpha, beta, p, dset_queue=None):
    ...
    try:
        rt, rx, save_data = main.single_pass(beta_c, alpha_c, 2, 300000, K=7, stoich_mat=stoich_mat, rvf=rvf, x0_g=x0_g,
                                             p=[alpha, beta])
        ...
        for b in betas:
            for a in alphas:
                try:
                    ...
                    for proc in procs:
                        ret = queue.get()
                        # print(f'data findings: {data_findings}')
                    ...
                    print()
                    print(
                        f'************************ Param complete! {round(a, 2)}  {round(b, 2)} ************************')
                    logger.info(
                        f'************************ Param complete! {round(a, 2)}  {round(b, 2)} ************************')
